...
export default function WhyItMattersSection({ className = '' }: WhyItMattersSectionProps) {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      if (!isMobile) {
        ...
        scrollTl
          .fromTo(panel, { x: '-70vw', opacity: 0 }, { x: 0, opacity: 1, ease: 'none' }, 0)
          .fromTo(image, { x: '70vw', opacity: 0, scale: 1.08 }, { x: 0, opacity: 1, scale: 1, ease: 'none' }, 0)
  ...
  return (
    ...
    >
      ...
      >
        <span ref={labelRef} className="font-mono-label text-[#2D6BFF] mb-6">{t.value_label}</span>
        ...
        <div ref={bodyRef} className={`${isMobile ? 'max-w-full' : 'max-w-[36vw]'} mb-10`}>
          <p className="text-[16px] text-[#A6B3D0] leading-relaxed mb-4">{t.value_body_1}</p>
          <p className="text-[16px] text-[#A6B3D0] leading-relaxed">{t.value_body_2}</p>
        </div>
        <button ref={ctaRef} onClick={scrollToContact} className="btn-pill-primary flex items-center gap-2 w-fit">
          {t.value_cta} <ArrowRight size={18} />
        </button>
      </div>
