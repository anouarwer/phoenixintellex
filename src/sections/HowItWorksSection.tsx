...
export default function HowItWorksSection({ className = '' }: HowItWorksSectionProps) {
  ...
  return (
    ...
    >
      ...
      >
        <span ref={labelRef} className="font-mono-label text-[#2D6BFF] mb-6">{t.method_label}</span>
        ...
        <div ref={bodyRef} className={`${isMobile ? 'max-w-full' : 'max-w-[34vw]'} mb-10`}>
          <p className="text-[16px] text-[#A6B3D0] leading-relaxed mb-4">{t.method_body_1}</p>
          <p className="text-[16px] text-[#A6B3D0] leading-relaxed">{t.method_body_2}</p>
        </div>
        <button ref={ctaRef} onClick={scrollToNext} className="btn-pill-outline flex items-center gap-2 w-fit">
          {t.method_cta} <ArrowRight size={18} />
        </button>
      </div>
    </section>
  );
